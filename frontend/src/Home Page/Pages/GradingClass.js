...
const GradingClass = () => {
  ...
  useEffect(() => {
    const fetchClassNumbers = async () => {
      try {
        ...
        const classNumbersArray = responseData.details[0].classNumber[0].split(",");
  ...
  const materialSubmitHandler = async (event) => {
    ...
    if (formState.inputs.classNumber.value) {
        const classNumber=encodeURIComponent(formState.inputs.classNumber.value);
        console.log(classNumber)
      navigate(`/Grades/${classNumber}`);
    }
  };

  return (
    <div className="gradingclass">
        ...
        <Card className="dropdown-card" hoverable>
          <form onSubmit={materialSubmitHandler}>
            ...
            <div className="submit-button-container">
              <button type="submit" id="submit" disabled={!formState.inputs.classNumber.value}>
                Submit
              </button>
            </div>
          </form>
